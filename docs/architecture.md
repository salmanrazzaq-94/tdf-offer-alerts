...
| `RUN_LOGS` | Optional persisted failure breadcrumbs for operator diagnostics |
...
The Worker treats KV as recoverable operational state, not a perfect database. Tests cover corrupted seen state, logs, metadata, auth state, health state, and delta lock recovery.
...
8. Write changed product state, emit a structured Cloudflare Workers Logs summary, and persist only failure breadcrumbs to `RUN_LOGS`.
...
The E2E job uses separate Cloudflare resources and secrets from production. It covers `/cookie`, `/verify-cookie`, `/run-delta`, `/run-daily`, operator `/debug` and `/logs`, supported Telegram commands, and `/refresh-failed`.
