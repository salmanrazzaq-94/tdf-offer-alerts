...
flowchart TD
  ...
  Commands --> Debug["/debug command"]
  Commands --> Logs["/logs command"]
...
| `RUN_LOGS` | Bounded structured run history |
| `AUTH_STATE` | Last auth failure, notification timestamp, and refresh-dispatch status |
| `HEALTH_STATE` | Stale-success notification state |
...
8. Write the refreshed seen state and structured run log.
...
The E2E job uses separate Cloudflare resources and secrets from production. It covers `/cookie`, `/verify-cookie`, `/run-delta`, `/run-daily`, `/debug`, `/logs`, Telegram commands, and `/refresh-failed`.
