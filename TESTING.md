...
- `npm run test:coverage` for deterministic tests with coverage counted across production modules.
- `npm run worker:check` for a Wrangler dry-run deployment bundle check.

PR tests do not call live TDF, Telegram, GitHub, Browserbase, or Cloudflare services. External boundaries are faked only where needed to make code paths deterministic and to assert request shape.
